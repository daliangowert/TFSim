...
int sc_main(int argc, char *argv[])
{
    ...
    string bench_name = "";
    int nadd, nmul, nls, n_bits, m_bits, bpb_size, cpu_freq;
    nadd = 3;
    nmul = nls = 2;
    n_bits = 2;
    m_bits = 3;
    ...
    spec_sub->append("Branch Prediction Buffer", [&](menu::item_proxy &ip)
        ...
        set_spec(plc, spec); });
    // Modo com o correlation_predict
    ...
    sub->append("Tamanho do BPB e Preditor", [&](menu::item_proxy &ip)
        ...
        inputbox::integer input_n("N_BITS", n_bits, 1, 3, 1);
        inputbox::integer input_m("M_BITS", m_bits, 1, 10, 1);
        if(ibox.show_modal(size, input_n, input_m)){
            bpb_size = size.value();
            n_bits = input_n.value();
            m_bits = input_m.value();
        } });
        ...
        if(fila)
        {
            ...
            if(spec){
                ...
                if(mode == 1)
                    ...
                    top1.rob_mode_cp(n_bits, m_bits, bpb_size, nadd,nmul,nls,instruct_time,instruction_queue,table,memory,reg,instruct,clock_count,rob);
            }
            else
